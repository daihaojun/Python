...
# come from https://github.com/piglei/zkpython_example/blob/master/zkclient.py
...
class ZKClient(object):
    ...
    def connection_watcher(self, h, type, state, path):
        ...
        if state == zookeeper.CONNECTED_STATE:
            ...
            self.handle = zookeeper.init(
                self.servers, self.connection_watcher, self.timeout)
        else:
            print('unexpected session event %s %s', h, state)
    ...
    def acreate(self, path, callback, data="",
                flags=0, acl=[ZOO_OPEN_ACL_UNSAFE]):
        result = zookeeper.acreate(
            self.handle, path, data, acl, flags, callback)
        return result
...
class SequentialCountingWatcher(CountingWatcher):
    ...
    def __call__(self, handle, typ, state, path):
        if not self.child_path(self.count) == path:
            raise ZKClientError(
                "handle %d invalid path order %s" % (handle, path))
...
class Callback(object):
    ...
    def waitForSuccess(self):
        ...
        if not self.callback_flag:
            raise ZKClientError(
                "asynchronous operation timed out on handle %d" %
                (self.handle))
